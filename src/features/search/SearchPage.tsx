  ...
  return (
    <div className="space-y-6">
        ...
        <InfoNote variant="info">
          <p>
            <strong>How Semantic Search Works:</strong> Unlike keyword search,
            semantic search understands the meaning behind your query. It uses
            AI embeddings to find documents that are conceptually similar, even
            if they don't share exact words. For example, searching "machine
            learning" will also find documents about "AI algorithms" or "neural
            networks".
          </p>
          <p className="mt-2">
            <strong>Similarity Score:</strong> Lower scores mean more similar
            (range: 0-2). Results are sorted by relevance. Use the threshold
            filter to control result quality—stricter thresholds return fewer
          ...
          {showThresholdControl && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-lg border border-border bg-card animate-in slide-in-from-top-2">
              ...
              <p className="text-xs text-muted-foreground flex-1">
                <strong>Stricter</strong> (lower values) = only very similar
                documents. <strong>Looser</strong> (higher values) = more
                results. Range: 0-2.
              </p>
            </div>
          )}
